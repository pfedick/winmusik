...
void EditTrack::setData(const TrackInfo& data)
{
    ...
    // Länge
    if (Ti.Length > 0)
        Tmp.setf("%0i:%02i", (int)(Ti.Length / 60), Ti.Length % 60);
    else
        Tmp.clear();
    ...
    if (Ti.Flags & 1)
        ...
        ui.complete->setChecked(false);

    if (Ti.Flags & 2)
        ui.realTitle->setChecked(true);
    else
        ui.realTitle->setChecked(false);

    if (Ti.Flags & 8)
        ui.interrupted->setChecked(true);
    else
...
const TrackInfo& EditTrack::getData()
{
    ...
    if ((Artist.isEmpty()) && (Title.isEmpty())) {
        // Warning
        int ret =
            QMessageBox::warning(this, tr("WinMusik: Attention"), tr("Artist and title are empty!\nDo you realy want to save this track?"),
    ...
    if (ui.complete->isChecked())
        ...
        data.Ti.Flags &= (0xff - 1);
    if (ui.realTitle->isChecked())
        data.Ti.Flags |= 2;
    else
        data.Ti.Flags &= (0xff - 2);
    if (ui.interrupted->isChecked())
        data.Ti.Flags |= 8;
    else
        data.Ti.Flags &= (0xff - 8);
 ...
 */
{
    ...
    if (key == Qt::Key_Escape) {
        ...
        // *************************************************************************** F6
    } else if (DeviceType==7 && key==Qt::Key_F6 && modifier==Qt::ControlModifier && fkeys->isEnabled(6)==true && position>3 &&
    wm->conf.DevicePath[DeviceType].NotEmpty()==true) { wm->TrashMP3File(DeviceId,Page,Track.Track); return
    on_f6_Pressed(target,Qt::NoModifier);

    } else if (DeviceType==7 && key==Qt::Key_F6 && fkeys->isEnabled(6)==true && position>3 &&
    wm->conf.DevicePath[DeviceType].NotEmpty()==true) { return on_f6_Pressed(target,modifier);

    } else if (DeviceType==7 && key==Qt::Key_F6 && fkeys->isEnabled(6)==true && position==3 &&
    wm->conf.DevicePath[DeviceType].NotEmpty()==true) { return on_f6_MassImport();
        // *************************************************************************** F7
        ...
        */
        // *************************************************************************** F9
        /*
    } else if (DeviceType==7 && key==Qt::Key_F9 && modifier==Qt::NoModifier && position==3 &&
    wm->conf.DevicePath[DeviceType].NotEmpty()==true) { return on_f9_UpdateAllID3Tags();
        // *************************************************************************** F10
    } else if (key==Qt::Key_F10 && position>2 && modifier==Qt::NoModifier && DeviceType==7 &&
    wm->conf.DevicePath[DeviceType].NotEmpty()==true) { return on_f10_WritePlaylist();
        // *************************************************************************** F11 => Printing
