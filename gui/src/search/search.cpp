 ...
 */
{
    ...
    if (type == QEvent::FocusIn) {
        if (target == ui.lengthStart || target == ui.lengthEnd || target == ui.qs_genre) {
            ((QLineEdit*)target)->selectAll();
        } else if (target == ui.energyStart || target == ui.energyEnd || target == ui.bpmStart || target == ui.bpmEnd) {
            ((QSpinBox*)target)->selectAll();
        } else if (target == ui.releaseDateStart || target == ui.releaseDateEnd || target == ui.recordDateStart ||
                   target == ui.recordDateEnd) {
            ((QDateEdit*)target)->selectAll();
        }
    } else if (type == QEvent::DragEnter) {
        if (target == ui.tabWidget || target == ui.query || target == ui.queryLayout) {
...
void Search::FastSearch(const ppl7::String& Artist,
                        const ppl7::String& Title,
                        const ppl7::String& Version,
                        const ppl7::String& Genre,
                        const ppl7::String& Tags,
                        const ppl7::String& Label)
{
...
void Search::SortResult(const std::list<const DataTitle*>& in, std::list<const DataTitle*>& out)
{
    ...
    for (it = in.begin(); it != in.end(); ++it) {
        ...
        sorter.insert(std::pair<ppl7::String, const DataTitle*>(ppl7::ToString("%08u:%08u", ti->ReleaseDate, ti->TitleId), ti));
    }
...
void Search::renderTrack(WMTreeItem* item, const DataTitle* ti)
{
    ...
    switch (ti->Rating) {
    case 0:
        item->setIcon(SEARCH_TRACKLIST_RATING_ROW, QIcon(":/bewertung/resources/rating-0.png"));
        item->setText(SEARCH_TRACKLIST_RATING_ROW, "0");
        break;
    case 1:
        item->setIcon(SEARCH_TRACKLIST_RATING_ROW, QIcon(":/bewertung/resources/rating-1.png"));
        item->setText(SEARCH_TRACKLIST_RATING_ROW, "1");
        break;
    case 2:
        item->setIcon(SEARCH_TRACKLIST_RATING_ROW, QIcon(":/bewertung/resources/rating-2.png"));
        item->setText(SEARCH_TRACKLIST_RATING_ROW, "2");
        break;
    case 3:
        item->setIcon(SEARCH_TRACKLIST_RATING_ROW, QIcon(":/bewertung/resources/rating-3.png"));
        item->setText(SEARCH_TRACKLIST_RATING_ROW, "3");
        break;
    case 4:
        item->setIcon(SEARCH_TRACKLIST_RATING_ROW, QIcon(":/bewertung/resources/rating-4.png"));
        item->setText(SEARCH_TRACKLIST_RATING_ROW, "4");
        break;
    case 5:
        item->setIcon(SEARCH_TRACKLIST_RATING_ROW, QIcon(":/bewertung/resources/rating-5.png"));
        item->setText(SEARCH_TRACKLIST_RATING_ROW, "5");
        break;
    case 6:
        item->setIcon(SEARCH_TRACKLIST_RATING_ROW, QIcon(":/bewertung/resources/rating-6.png"));
...
void Search::configureFilter(ResultFilter& filter)
{
    ...
    if (ui.enableGenreSearch->isChecked()) filter.setGenres(true, ui.qs_genre->text());
    if (ui.enableBpmSearch->isChecked()) filter.setBpmRange(true, ui.bpmStart->value(), ui.bpmEnd->value());
...
void Search::on_trackList_customContextMenuRequested(const QPoint& pos)
{
    ...
    if (column == SEARCH_TRACKLIST_RATING_ROW) {
        ...
        if (t != NULL && (t->Flags & 16) == 0)
            a = m->addAction(QIcon(":/icons/resources/musicKeyOk.png"), tr("Music Key is verified", "trackList Context Menue"), this,
                             SLOT(on_contextMusicKeyVerified_triggered()));
        else if (t != NULL && (t->Flags & 16) == 16)
            a = m->addAction(QIcon(":/icons/resources/musicKeyNotOk.png"), tr("Music Key is not verified", "trackList Context Menue"), this,
                             SLOT(on_contextMusicKeyVerified_triggered()));
        ...
        a = m->addAction(QIcon(":/icons/resources/frame_text.png"), tr("select all", "trackList Context Menue"), this,
                         SLOT(on_markAllButton_clicked()));
        m->addAction(QIcon(":/icons/resources/eraser.png"), tr("select none", "trackList Context Menue"), this,
                     SLOT(on_markNoneButton_clicked()));

        m->addSeparator();
        m->addAction(QIcon(":/icons/resources/findmore.png"), tr("Find other versions", "trackList Context Menue"), this,
        ...
        m->addAction(QIcon(":/icons/resources/findmore-title.png"), tr("Find other artists of this title", "trackList Context Menue"), this,
                     SLOT(on_contextFindMoreTitle_triggered()));
        if (t)
            m->addAction(wm->GetDeviceIcon(t->DeviceType), tr("All tracks on this medium", "trackList Context Menue"), this,
                         SLOT(on_contextFindMoreMedium_triggered()));
        m->addSeparator();
        if (DeviceType == 7)
...
#ifdef _WIN32
                // if (wmlog) wmlog->Printf(ppl6::LOG::DEBUG, 10, "Search", "on_trackList_MouseMove", __FILE__, __LINE__, "Add File to Drag:
                // %s", (const char*)File);
...
void Search::on_ClipBoardTimer_update()
{
    ...
    if (wm->RegExpCapture.match(clip, match)) {
        match.Artist = ppl7::RegEx::replace("/\\(.{2,3}?\\)/is", match.Artist, ""); // Filter out county code additions in artist ?
...
void Search::RecursiveDirSearch(ppl7::Array& search, const ppl7::String& dir)
{
    ...
    while (Dir.getNext(entry, it)) {
        if (entry.isDir()) {
            if (entry.Filename != "." && entry.Filename != "..") RecursiveDirSearch(search, entry.File);
...
void Search::handleDropEvent(QDropEvent* event)
{
    ...
    /*
    if (mime->hasUrls()) {
        ppl7::PrintDebug("Wir haben URLs\n");
    } else if (mime->hasText()) {
        ppl7::PrintDebug("Wir haben Text\n");
    } else if (mime->hasHtml()) {
        ppl7::PrintDebug("Wir haben HTML\n");
    } else if (mime->hasImage()) {
        ppl7::PrintDebug("Wir haben ein Bild\n");
    } else {
        ppl7::PrintDebug("Wir haben sonstiges\n");
        for (int i = 0; i < mime->formats().size(); i++) {
            ppl7::PrintDebug("Format: %s\n", (const char*)ppl7::String(mime->formats()[i]));
        }
    }
        */
