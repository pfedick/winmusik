...
void Edit::UpdateDevice()
{
    ...
    Tmp.setf("%s\n%s", (datadevice.Title.notEmpty() ? (const char*)datadevice.Title : ""),
...
void Edit::FillEditFields()
{
    ...
    if (Ti.Flags & 16)
        ui.musickeyVerified->setChecked(true);
    else
        ui.musickeyVerified->setChecked(false);
...
void Edit::RenderTrack(WMTreeItem* item, const DataTitle& title)
{
    ...
    Text.setf("%s - %s", (title.Artist.notEmpty() ? (const char*)title.Artist : unknown),
...
void Edit::SaveEditorTrack()
{
    ...
    if (ui.interrupted->isChecked())
        ...
        Ti.Flags &= (0xff - 8);
    if (ui.musickeyVerified->isChecked())
        Ti.Flags |= 16;
    else
        Ti.Flags &= (0xff - 16);
...
void Edit::importFromCddb()
{
    ...
    if (matches.size() < 1) {
        QMessageBox::information(NULL, tr("audio cd not found"), tr("The audio cd was not found in the internet cd database"),
...
void Edit::renumber()
{
    ...
    if (newDeviceId < 1) {
        QMessageBox::critical(NULL, tr("Error"), tr("Invalid number"), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    for (int i = track + 1; i <= TrackList.GetMax(); i++) {
        if (TrackList.Exists(i)) {
            ...
            if (new_track.TitleId > 0 && wm->TitleStore.Exists(new_track.TitleId)) {
                ...
                if (Ti.DeviceId == new_track.DeviceId && Ti.DeviceType == new_track.Device && Ti.Page == new_track.Page) {
