    ...
    : QDialog(parent)
{
    ...
    ui.SearchListCoverPath->setText(c->SearchListCoverPath);
    ...
    if (c->MaxFilenameLength <= 64)
        ui.maxFilenameLength->setCurrentIndex(0);
    else if (c->MaxFilenameLength <= 100)
        ui.maxFilenameLength->setCurrentIndex(1);
    else if (c->MaxFilenameLength <= 207)
        ui.maxFilenameLength->setCurrentIndex(2);
    else if (c->MaxFilenameLength <= 255)
        ui.maxFilenameLength->setCurrentIndex(3);
    else
        ui.maxFilenameLength->setCurrentIndex(4);
    ...
    for (it = c->DirectorySearch.begin(); it != c->DirectorySearch.end(); ++it) {
        ui.directoryList->addItem((*it));
    }
    ...
    case musicKeyTypeNone:
        ui.saveMusicKey->setCurrentIndex(0);
        break;
    case musicKeyTypeMusicalSharps:
        ui.saveMusicKey->setCurrentIndex(1);
        break;
    case musicKeyTypeMusicalFlats:
        ui.saveMusicKey->setCurrentIndex(2);
        break;
    case musicKeyTypeOpenKey:
        ui.saveMusicKey->setCurrentIndex(3);
        break;
    case musicKeyTypeCustom:
        ui.saveMusicKey->setCurrentIndex(4);
        break;
    default:
        ui.saveMusicKey->setCurrentIndex(3);
        break;
    ...
    if (ppl7::AudioCD::isSupported() == true && ppl7::CDDB::isSupported() == true) {
        ...
        ui.cdioDevice->clear();
        std::list<ppl7::String> devices = ppl7::AudioCD::getDevices();
...
ppl7::String Properties::GetLocale()
{
    ppl7::String Tmp;
...
int Properties::Save()
{
    ...
    c->SearchListCoverPath = ui.SearchListCoverPath->text();
    ...
    if (ui.maxFilenameLength->currentIndex() == 0)
        c->MaxFilenameLength = 64;
    else if (ui.maxFilenameLength->currentIndex() == 1)
        c->MaxFilenameLength = 100;
    else if (ui.maxFilenameLength->currentIndex() == 2)
        c->MaxFilenameLength = 207;
    else if (ui.maxFilenameLength->currentIndex() == 3)
        c->MaxFilenameLength = 255;
    else
        c->MaxFilenameLength = 1024;
    ...
    for (int i = 0; i < ui.directoryList->count(); i++) {
        ...
        Tmp = item->text();
        c->DirectorySearch.push_back(Tmp);
    }

    // MusicKey
    switch (ui.displayMusicKey->currentIndex()) {
    case 0:
        c->musicKeyDisplay = musicKeyTypeMusicalSharps;
        break;
    case 1:
        c->musicKeyDisplay = musicKeyTypeMusicalFlats;
        break;
    case 2:
        c->musicKeyDisplay = musicKeyTypeOpenKey;
        break;
    case 3:
        c->musicKeyDisplay = musicKeyTypeCustom;
        break;
    default:
        c->musicKeyDisplay = musicKeyTypeOpenKey;
        break;
    ...
    case 0:
        c->musicKeyTag = musicKeyTypeNone;
        break;
    case 1:
        c->musicKeyTag = musicKeyTypeMusicalSharps;
        break;
    case 2:
        c->musicKeyTag = musicKeyTypeMusicalFlats;
        break;
    case 3:
        c->musicKeyTag = musicKeyTypeOpenKey;
        break;
    case 4:
        c->musicKeyTag = musicKeyTypeCustom;
        break;
    default:
        c->musicKeyTag = musicKeyTypeOpenKey;
        break;
    ...
    try {
        ...
        ShowException(exp, tr("Could not save configuration!"));
        return 0;
    }
    wm->InitLogging();
    wm->UpdateMenue();
...
void Properties::on_wmDataPathButton_clicked()
{
    ...
    QString dir = QFileDialog::getExistingDirectory(
        this, tr("Select directory for WinMusik database.\nAttention: Changing this directory can result in loss of data!"), p,
...
void Properties::on_devicePathButton_clicked(int device, QLineEdit* line)
{
    ...
    QString dir = QFileDialog::getExistingDirectory(this, Tmp, p, QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    if (dir.length()) {
        line->setText(dir);
        Change();
    }
...
void Properties::on_mp3PlayerButton_clicked()
{
    ...
    QString file = QFileDialog::getOpenFileName(this, tr("Select the program with which you want to use for playing MP3-Files"), p);
    if (file.length()) {
...
void Properties::on_aiffPlayerButton_clicked()
{
    ...
    QString file = QFileDialog::getOpenFileName(this, tr("Select the program with which you want to use for playing AIFF-Files"), p);
    if (file.length()) {
...
void Properties::on_tmpPathButton_clicked()
{
    ...
    QString dir = QFileDialog::getExistingDirectory(this, tr("Select directory for temporary files"), p,
                                                    QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    if (dir.length()) {
...
void Properties::on_SearchListCoverPathButton_clicked()
{
    QString p = ui.SearchListCoverPath->text();
    if (p.length() == 0) {
        p = QDir::tempPath();
    }
    QString dir = QFileDialog::getExistingDirectory(this, tr("Select directory for searchlist covers"), p,
                                                    QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    if (dir.length()) {
        ui.SearchListCoverPath->setText(dir);
        Change();
    }
}
...
void Properties::on_logfileButton_clicked()
{
    ...
    QString dir = QFileDialog::getSaveFileName(this, tr("Select location and filename for debug logfile"), p, "*.log");
    if (dir.length()) {
...
void Properties::on_addDirectory_clicked()
{
    ...
    p = LastHDSearchDir;
    QString dir = QFileDialog::getExistingDirectory(this, tr("Select the directory you want to add to the harddisk search"), p,
                                                    QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
    if (dir.length()) {
        LastHDSearchDir = dir;
