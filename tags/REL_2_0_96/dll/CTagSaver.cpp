...
void CTagSaver::Add(char *filename, ppl6::CpplAssocArray *Tags)
{
	...
	Todo.Set("[]",&a);
	Log.Printf(ppl6::LOG::DEBUG,2,"Adding File %s", filename);
	//Log.PrintArray(ppl6::LOG::DEBUG,1,Tags,"CTagSaver::Add tags");
	//Log.PrintArray(ppl6::LOG::DEBUG,1,&Todo,"CTagSaver::Add Todo");
	if (!ThreadIsRunning()) {
		Log.Printf(ppl6::LOG::DEBUG,1,"Starte Thread");
		ThreadStart();
		Log.Printf(ppl6::LOG::DEBUG,1,"Starte Thread - ok");
	}
	Mutex.Unlock();
...
int CTagSaver::UpdateNow(char *filename, ppl6::CpplAssocArray *Tags)
{
	Log.Printf(ppl6::LOG::DEBUG,1,"UpdateNow: %s",filename);
	Log.PrintArray(ppl6::LOG::DEBUG,1,Tags,"CTagSaver::UpdateNow tags");

	ppl6::CID3Tag Tag;
	ppluint32 e;
	if (!Tag.Load(filename)) {
		e=GetErrorCode();
		Log.Printf(ppl6::LOG::DEBUG,1,"UpdateNow Tag.Load failed: %u",e);
		if (e<402 || e> 405) {
			Log.LogError();
			return 0;
		}
	...
	Log.Printf(ppl6::LOG::DEBUG,1,"Saving: %s",filename);
	if (!Tag.Save()) {
		ppl6::PushError();
		Log.LogError();
		Log.Printf(ppl6::LOG::DEBUG,1,"Failed: %s",filename);
		ppl6::PopError();
		return 0;
	}
	Log.Printf(ppl6::LOG::DEBUG,1,"Done: %s",filename);
	return 1;
}

void CTagSaver::ThreadMain(void *param)
{
	Log.Printf(ppl6::LOG::DEBUG,1,"ThreadMain");
	...
	while (1) {
		...
		if (!Job) {
			...
			Todo.Delete(&key);
			Mutex.Unlock();
			Log.Printf(ppl6::LOG::DEBUG,1,"Key: >>%s<<",(char*)key);
			Log.PrintArray(ppl6::LOG::DEBUG,1,&MyJob,"MyJob");
			if (!UpdateNow(MyJob.Get("filename"),MyJob.GetArray("tags"))) {
				// Update hat nicht funktioniert
				Mutex.Lock();
				MyJob.Setf("retry","%llu",ppl6::GetTime()+60);
				Todo.Set(&key,&MyJob);
				Log.Printf(ppl6::LOG::DEBUG,1,"Retry");
				Mutex.Unlock();
			}
		}
	}
