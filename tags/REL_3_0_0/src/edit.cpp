 ...
 * $Id: edit.cpp,v 1.1 2010/05/16 12:23:30 pafe Exp $
 *
 *
 ...
 */
{
	...
	InstallFilter(ui.complete,22);
	InstallFilter(ui.realTitle,23);
	InstallFilter(ui.interrupted,24);
	InstallFilter(ui.channels,25);
	InstallFilter(ui.quality,26);
	InstallFilter(ui.rating,27);
	InstallFilter(ui.f12,28);
 ...
 */
{
    ...
    trackList->headerItem()->setText(0, tr("Track","trackList"));
    trackList->headerItem()->setText(1, tr("Artist - Title","trackList"));
    trackList->headerItem()->setText(2, tr("Version","trackList"));
    trackList->headerItem()->setText(3, tr("Genre","trackList"));
    trackList->headerItem()->setText(4, tr("Length","trackList"));
 ...
 */
{
	...
	trackList->setColumnWidth(4,80);
	w=w-140;
	trackList->setColumnWidth(1,w*55/100);
	trackList->setColumnWidth(2,w*30/100);
	trackList->setColumnWidth(3,w*15/100);
...
void Edit::UpdateFkeys()
{
	...
	switch (position) {
		...
		case 3:		// Device Track
			...
			if (DeviceType==7 && wm->conf.MP3Path.NotEmpty()==true) SetFkey(ui.f9,":/fkeys/resources/fkeys/f-key-2009.png",t[16]);
			break;
		case 4:		// Interpret
	...
	if (position>2) {
		...
		if (DeviceType==7 && wm->conf.MP3Path.NotEmpty()==true) {
 ...
 */
{
	switch (position) {
		...
		case 21: ui.f12->setFocus(); break;
		//case 21: ui.complete->setFocus(); break;
		case 22: ui.realTitle->setFocus(); break;
		case 23: ui.interrupted->setFocus(); break;
		case 24: ui.channels->setFocus(); break;
		case 25: ui.quality->setFocus(); break;
		case 26: ui.rating->setFocus(); break;
		case 27: ui.f12->setFocus(); break;
	};
}
 ...
 */
{
	...
	if (key==Qt::Key_Escape) {
		...
		// *************************************************************************** F6
	} else if (DeviceType==7 && key==Qt::Key_F6 && ui.f6->isEnabled()==true && position>3 && wm->conf.MP3Path.NotEmpty()==true) {
		...
		return true;
		// *************************************************************************** F9
	} else if (DeviceType==7 && key==Qt::Key_F9 && modifier==Qt::NoModifier && position==3 && wm->conf.MP3Path.NotEmpty()==true) {
		return on_f9_UpdateAllID3Tags();
		// *************************************************************************** F10
	} else if (key==Qt::Key_F10 && position>2 && modifier==Qt::NoModifier && DeviceType==7 && wm->conf.MP3Path.NotEmpty()==true) {
...
bool Edit::on_f6_Pressed(__unused__ QObject *target, int modifier)
{
	...
	if (Path.IsEmpty()) {
		...
		if (Path.IsEmpty()) {
			QMessageBox::information(this, tr("WinMusik: Hinweis"),
					tr("Es sind keine weiteren Titel ohne Index im Verzeichnis\ndes Tonträgers vorhanden"));
			return false;
		}
	}
	...
	if (modifier==Qt::NoModifier) {
		...
		if (wm->conf.ReadId3Tag==1) return CopyFromID3v1Tag(Path,File);
		else if (wm->conf.ReadId3Tag==2) return CopyFromID3v2Tag(Path,File);
		return CopyFromFilename(Path);
	}
...
bool Edit::on_f10_WritePlaylist()
{
	...
	wm->RaiseError(this,tr("Playlists konnten nicht erstellt werden"));
	return true;
}
...
bool Edit::on_trackList_MouseMove(QMouseEvent *event)
{
	...
	QPixmap Icon;
	Icon.load(":/devices48/resources/tr48x48-0007.png");
    drag->setPixmap(Icon);
...
void Edit::SaveTrack()
{
	...
	if (!wm->TitleStore.Put(&Ti)) {
		wm->RaiseError(this,tr("Konnte Titel nicht in TitleStore speichern"));
		ui.artist->setFocus();
	...
	if (!TrackList->Put(&Track)) {
		wm->RaiseError(this,tr("Konnte Track nicht in TrackList speichern"));
		ui.artist->setFocus();
		return;
	}
...
void Edit::on_trackList_itemClicked (__unused__ QTreeWidgetItem * item, __unused__ int column )
{
	...
	if (t) {
		...
		Text.Setf("%s - %s (%s, %0i:%02i min, %s)",t->Artist,t->Title,
			wm->GetVersionText(t->VersionId), t->Length/100,t->Length%100, wm->GetGenreText(t->GenreId));
