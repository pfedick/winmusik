 ...
 * $Id: search.cpp,v 1.1 2010/05/16 12:23:30 pafe Exp $
 *
 *
...
void Search::FastSearch(const char *artist, const char *title)
{
	ui.artist->setText("");
	ui.title->setText("");
	if (artist) ui.artist->setText(artist);
	...
	CTitleHashTree res;
	if (wm->Hashes.Find(artist,title,res)) {
		Tmp.Setf("%u",res.Num());
			...
			if (ti->DeviceType<20 && AllowedDevices[ti->DeviceType]==1) {
				...
				if (Artist.NotEmpty()) {
					if (SArtist.Instr(Artist)>=0) {
						if (Title.IsEmpty()) Results.Add(ti);
						else if (STitle.Instr(Title)>=0) Results.Add(ti);
					}
				} else if (Title.NotEmpty()) {
					if (STitle.Instr(Title)>=0) Results.Add(ti);
				}
			}
		}
	}
...
void Search::on_querysearchButton_clicked()
{
	ui.numTracks->setText("");
	currentTrackListItem=NULL;
	trackList->clear();
	...
	ui.progressBar->setValue(0);
	CTitleHashTree res;
	if (wm->Hashes.FindGlobal(Query,res)) {
		Tmp.Setf("%u",res.Num());
		ui.numTracks->setText(Tmp);
	}
	ui.progressBar->setValue(30);
...
{
	// TODO: Das müssen wir noch einbauen
	ui.artist->setFocus();
}
