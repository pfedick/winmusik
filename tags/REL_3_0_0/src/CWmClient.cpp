 ...
 * $Id: CWmClient.cpp,v 1.1 2010/05/16 12:23:30 pafe Exp $
 *
 *
...
void CWmClient::OpenEditor(int devicetype)
{
...
{
	if (wmlog) wmlog->Printf(ppl6::LOG::DEBUG,3,"CWMClient","MP3Filename",__FILE__,__LINE__,"Nächste MP3-Datei suchen: DeviceId=%u, Page=%u, Track=%u",DeviceId,Page,Track);
	ppl6::CString Path, Pattern;
	if (conf.MP3Path.IsEmpty()) return Path;
	Path=conf.MP3Path;
	Path.RTrim("/");
	Path.RTrim("\\");
...
{
	ppl6::CString Filename;
	if (conf.MP3Path.IsEmpty()) return Filename;
	Filename=conf.MP3Path;
	Filename.RTrim("/");
	Filename.RTrim("\\");
	...
	for (int i=min;i<=max;i++) {
		...
		if (track) {
			...
			if (Ti) {
				...
				txt.Putsf("%3u. %s, %0i:%02i %s)\r\n",i,(const char*)Tmp,(int)(Ti->Length/60),Ti->Length%60,(const char*)Minuten);
...
void CWmClient::NormalizeTerm(ppl6::CString &term)
{
	...
	term.Replace(" versus "," ");
	term.Replace(" pres. "," ");
	term.Replace(" presents "," ");
	term.Replace(" vs. "," ");
	term.Replace(" ft. "," ");
	term.Replace(" feat. "," ");
	term.Replace(" featuring "," ");
	term.Replace(" und "," ");
	term.Replace(" and "," ");
	term.Replace("DJ ","");
	term.Trim();
