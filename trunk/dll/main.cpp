...
INT APIENTRY GetMP3Info (char *filename, MP3HEADER *mp3, ID3TAG *id3) {
	CpplFile ff;
	int ret;
	...
	if (ff.Open(tmpfile,"rb")) {
		ret=GetMP3Info_internal(&ff,mp3,id3);
		ff.Close();
		free(tmpfile);
		return (ret);
...
INT APIENTRY GetTitle (ppldd id, TITELDATEIALIGN *Ti)
{
